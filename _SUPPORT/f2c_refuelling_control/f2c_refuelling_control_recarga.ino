...
#define BATTERY_CHECK_INTERVAL 3000
#define PRESSURE_MIN 0.1 // bar
#define PRESSURE_CHARGE 0.12
...
void init_mode() {
    float targetPressureBar = mapPressure(analogRead(POT_PIN));
    if (targetPressureBar <= PRESSURE_CHARGE) {
        for (int i = 0; i < 10; i++) {
...
void battery_task(void* parameter) {
    ...
    while(true) {
        ...
        batteryVoltage = vOut * BATTERY_DIVIDER;

        digitalWrite(BATTERY_LED_PIN, batteryLow ? HIGH: LOW);
        batteryVoltage < BATTERY_LOW ? batteryLow = true: batteryLow = false;
